...
可通过「启用章回正则匹配」开关控制是否使用章节标题识别。开启时系统按正则匹配到的章节边界切分，并以单章节优先：即使多个短章节合计未超过「每块字数」，也不会自动合并为同一块。

**每块字数** 控制最大分块大小（默认 15000 字符）。开启章回正则时，仅当单个章节超过该大小才会进一步切分为「章节名-1 / 章节名-2」；关闭章回正则或未检测到章节时，则按该大小对全文进行普通分块。修改字数或正则设置后点击「重新分块」生效。
...
也可以点击「添加章节/记忆」手动创建队列项。没有上传 TXT 时，可直接粘贴章节原文或记忆片段开始转换；系统会自动显示章节队列并启用开始按钮。手动添加时可选择追加到末尾、插入到开头，或插入到指定章节之后。

### 章节操作

点击任意章节打开内容查看/编辑弹窗：

- **添加章节/记忆** — 手动输入标题和内容，加入章节队列
- **查看原文** — 完整章节内容
- **编辑** — 修改章节文本后保存
- **复制** — 复制原文到剪贴板
